...
import { TCaseItem } from "@/types";
import parse from "html-react-parser";

export function CaseStudies({ items }: { items: TCaseItem[] }) {
  const containerRef = useRef<HTMLDivElement | null>(null);
  const [mounted, setMounted] = useState(false);
  const [active, setActive] = useState<TCaseItem | null>(null);
  ...
  const openModal = (item: TCaseItem) => setActive(item);
          ...
          >
            ...
            >
              ...
              <div className="max-h-[88vh] overflow-auto rounded-2xl bg-white shadow-2xl dark:bg-neutral-950">
                {active.content ? (
                  <div className="w-full richtext p-5 md:p-10 space-y-5">
